...
export const DentalProductList = () => {
  ...
  const copyPricelist = () => {
    const text = pricelistItems
      .map((p: DentalProduct) => `${p.name} — ${formatPrice(p.price)}`)
      .join("\n");
    ...
    >
      <Table {...tableProps} rowKey="id" scroll={{ x: "max-content" }}>
        ...
        <Table.Column
          ...
          render={(text: string) =>
            text ? (
              <div style={{ maxWidth: 260, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }} title={text}>
                {text}
              </div>
            ) : null
          }
        />
        <Table.Column title="מחיר" dataIndex="price" render={(p: number) => formatPrice(p)} width={120} />
        <Table.Column
          title="במחירון"
        ...
        />
        <Table.Column title="עודכן" dataIndex="updated_at" render={(d: string) => formatDate(d)} width={140} />
        <Table.Column
          title="פעולות"
          width={100}
