...
import type { AestheticInquiry } from "../../../interfaces";
import {
  inquiryStatusColor,
  inquiryStatusLabel,
  formatDateOnly,
...
export const AestheticInquiryList = () => {
  ...
  return (
    ...
    >
              ...
              sortedSources.length === 0 ? (
                ...
                <Space size={[8, 8]} wrap>
                  {sortedSources.map(([src, count]) => {
                    ...
                    return (
                      <Tag key={src} color="purple" style={{ fontSize: 14, padding: "4px 10px" }}>
                        {src}: <b>{count}</b> ({pct}%)
                      </Tag>
                    );
                  })}
      ...
      <Table
        ...
        onRow={(record: AestheticInquiry) => ({
          onClick: (e) => {
            const target = e.target as HTMLElement;
            if (target.closest("button") || target.closest(".ant-btn")) return;
            navigate(`/aesthetic/inquiries/edit/${record.id}`);
          },
        ...
        columns={[
          ...
          },
          {
            title: "פעולות",
            width: 100,
            render: (_: unknown, record: AestheticInquiry) => (
              <Space>
